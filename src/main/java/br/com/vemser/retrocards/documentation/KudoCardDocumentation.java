...
public interface KudoCardDocumentation {
    ...
    @ApiResponses(
            value = {
                    @ApiResponse(responseCode = "200", description = "Sucess! Returns the list of all kudocards."),
                    @ApiResponse(responseCode = "403", description = "Invalid Permission! You do not have permission to acesses."),
                    @ApiResponse(responseCode = "400", description = "Bad Request! Invalid parameters"),
                    @ApiResponse(responseCode = "500", description = "Error! Could not connect to the server.")
            }
    ...
    @GetMapping("/list")
    ResponseEntity<PageDTO<KudoCardDTO>> listAll(Integer pagina, Integer registros) throws NegociationRulesException;
